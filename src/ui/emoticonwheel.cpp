                          ...
                          const QPixmap &skinAtlas) const
{
    ...
    // ── 背景：外环 + 内环分隔（半透明）──
    QPainterPath path;
    path.addEllipse(c, kOuterBgR, kOuterBgR);
    p.fillPath(path, QColor(30, 30, 40, 180));
    QPainterPath inner;
    inner.addEllipse(c, kInnerR, kInnerR);
    p.fillPath(inner, QColor(60, 60, 80, 200));

    // ── 中心取消 ──
    p.setPen(QPen(QColor(255, 255, 255, 220), 4.0));
    const double s = 14.0;
    ...
    if (!emoticonAtlas.isNull()) {
        ...
        for (int i = 0; i < NUM_EMOTICONS; ++i) {
            ...
            const QPointF pos = c + QPointF(std::cos(angle), std::sin(angle)) * kOuterItemR;
            const bool hover = (i == m_selEmoticon);
            const double size = hover ? 74.0 : 52.0;
    ...
    if (!skinAtlas.isNull()) {
        for (int i = 0; i < NUM_EYES; ++i) {
            ...
            const QPointF pos = c + QPointF(std::cos(angle), std::sin(angle)) * kEyeR;
            const bool hover = (i == m_selEye);
            const double size = hover ? 56.0 : 40.0;
            const QRectF src(kEyeRegionX[i], kEyeRegionY, kEyeRegionSize, kEyeRegionSize);
            if (i == 5) {
                // BLINK：垂直压扁
