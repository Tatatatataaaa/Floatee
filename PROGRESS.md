...
- **状态（2026-08-10）**：**M0 完成**——`online` 分支已建；WSH 与 Eye Care 已在分支内关闭（配置强制 false + 托盘屏蔽）；Node.js 服务器工程 `server/` 完成（config/protocol/store/session/rooms/admin/logger，**19/19 单测通过**，TCP 端到端 hello/create_room/error 验证通过）；客户端已加 **NetClient（QTcpSocket + JSON 行）+ 托盘 Network Test**（连本地 8764 → hello → create_room 弹窗，编译通过待 GUI 实测）。**待确认：圆盘附加热键默认不绑定（已定）**
