...
- **主动入口（问题 4，沿用 DDNet/QmClient `CEmoticon` 方案 + 交互定制）**：参考 [`EMOTICON_WHEEL.md`](./EMOTICON_WHEEL.md)，在画布内实现**表情圆盘**（overlay，不开第二窗口）：
...
| **M4 表情转发** ✅（2026-08-10） | 表情圆盘（外环 16 表情 + 内环 6 眼睛 + 点击式 + 数字键 + 托盘 Emoticon）；`emoticon`/`peer_emoticon` 转发 + 远端多 Tee 并行动画 + 远端右键管理菜单（隐藏/重置/踢出[房主权限]） | 一方发表情，他人对应 Tee 上方实时播放；双设备实测待做 |
...
- [x] **表情圆盘**：沿用 DDNet/QmClient `CEmoticon` 方案（[`EMOTICON_WHEEL.md`](./EMOTICON_WHEEL.md)）——同心圆（外环 16 表情 + 内环 6 眼睛）、角度+半径分层、弹簧+渐次 reveal 动画；**交互定制**：右键在**本地 Tee** 上打开（中心=本地 Tee 中心）+ 点击式提交，热键作附加（默认不绑定）；**右键切眼功能关闭**（眼睛走圆盘内环）；**远端 Tee 右键走独立管理菜单**（隐藏/重置/踢出[需房主权限]）（§6.4/§6.6）；
